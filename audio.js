/**
 * BIOS Sound Engine — Full Edition
...
window.BiosAudio = (function () {
  ...
  /* ── Virus alarm — rapid descending beeps ──────────── */
  function virusAlarm() {
    try {
      const c = getCtx();
      for (let i = 0; i < 8; i++) {
  ...
  function fanSpinUp(durationMs = 1800) {
    try {
      ...
      // Low-pass filter — starts very low, sweeps up (motor spinning up)
  ...
  /* ── CRT degauss — deep magnetic WHOOM ─────────────── */
  function degauss() {
    try {
      const c = getCtx();
  ...
  function crtOn() {
    try {
      ...
      const hum = c.createOscillator(); const humGain = c.createGain();
      hum.connect(humGain); humGain.connect(c.destination);
      hum.type = 'sine'; hum.frequency.setValueAtTime(60, c.currentTime);
  ...
  return { postBeep, errorBeep, virusAlarm, tick, hdSeek, memTick, fanSpinUp, degauss, crtOn, crtOff, navBeep, beep };
})();
